...
public class UserServiceImpl extends BaseService implements UserService {
    ...
    //Thêm mới một user
    @Override
    public long save(UserDto dto, MultipartFile file) {
...
//		List<UserDto> dtos = userRepository.findAllUserOfCourse(id);
    ...
    @Override
    public void registerAdminUser(UserDto dto) {
        // MAPPING USER DTO SANG USER ENTITY
        Role roleAdmin = roleRepository.findByName(Consts.RoleName.ROLE_ADMIN);
        ...
        entity.setPassword(BCrypt.hashpw(dto.getPassword(), BCrypt.gensalt()));
        entity.setFullname(dto.getFullname());
        entity.setRoleId(roleAdmin.getId());
    ...
    @Override
    public UsersModel search(UserSearchDto searchDto) throws ExecutionException, InterruptedException {

        UsersModel model = new UsersModel();
        List<UserDto> users = userRepository.findUserByCondition(searchDto);
        AppUtils.createOrderNumber(users, searchDto.getPageIndex(), searchDto.getPageSize() );
        model.setTotal(userRepository.findTotalResult(searchDto));
        model.setUsers(users);
